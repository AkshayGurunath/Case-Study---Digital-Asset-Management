        ...
        public bool ReserveAsset(int assetId, int employeeId, string reservationDate, string startDate, string endDate)
        {
            try
            {
                using (var connection = GetConnection())
                {
                    ...
                    // Step 3: Retrieve the current maximum reservation_id
                    ...
                    // Step 4: Insert the new reservation with the newly generated reservation_id
            ...
            {
                Console.WriteLine($"AssetNotFoundException caught: {ex.Message}");
                throw; // Rethrow custom exception if already handled
            }
